...
public class OrderedList_inArraySlots
     ...
     */
     private int champIndex( ArrayList<Integer> challengers) {
        ...
        int minValue;
        int index;
        for( index = 0, minValue = 0
           ; index < challengers.size() && minIndex == -1
           ; index++) {
             if (challengers.get(index) != null) {
                minIndex = index;
                minValue = challengers.get(index);
                System.out.println("Debugging: " + index);
              }
           }
        for(
           ; index < challengers.size()
           ; index++
           ){
             if (challengers.get(index) != null && challengers.get(index) < minValue) {
               minIndex = index;
               minValue = challengers.get(index);
             }
           }
